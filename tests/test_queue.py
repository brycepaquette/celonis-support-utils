...
def test_queue_add(queue, sample_salesforce_payload):
    new_ticket = Ticket.from_salesforce_payload(sample_salesforce_payload)
    queue.add(new_ticket)
    assert len(queue._queue) == 1
    assert queue._queue[0] == new_ticket


def test_queue_next(queue, sample_salesforce_payload):
    new_ticket = Ticket.from_salesforce_payload(sample_salesforce_payload)
    queue.add(new_ticket)
